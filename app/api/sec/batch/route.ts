...
async function ingestSecTicker(
  ticker: string,
  cikStr: string,
...
export async function GET() {
  ...
  // 2. Fetch EDGAR ticker→CIK map once
  const cikMapRes = await fetch('https://www.sec.gov/files/company_tickers.json', {
    headers: SEC_HEADERS,
  })
  if (!cikMapRes.ok) {
    return NextResponse.json({ error: 'Failed to fetch company_tickers.json' }, { status: 502 })
  }
  ...
  // 3. Process in batches of 3 with 2s delay between batches
  ...
  // Tickers with no CIK mapping count as failed
